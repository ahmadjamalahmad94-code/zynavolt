      ...
      body: DecoratedBox(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            // v100-fonts (TEMPORARY) — runtime Arabic-font picker.
            // Lets the owner preview Cairo / Tajawal / Almarai / IBM
            // Plex / Readex / Noto Arabic / El Messiri before picking
            // a permanent default. Remove this card + the
            // `font_pref.dart` provider + the `google_fonts`
            // dependency once a winner is chosen.
            const _FontCard(),
            const SizedBox(height: 12),
            const _LanguageCard(),
...
/// v100-fonts (TEMPORARY) — Arabic-font picker. Renders one row per
/// `AppFontPref` value with the font's name + a sample sentence
/// rendered IN that font, so the owner can compare the typefaces
/// side-by-side before picking a permanent default. Selecting a row
/// updates `appFontPrefProvider`, which re-builds the whole
/// `MaterialApp` with a fresh `ThemeData` keyed off the new font.
///
/// **REMOVE THIS WIDGET** along with `font_pref.dart`, the
/// `google_fonts` dependency, and the matching pubspec block once
...
class _FontCard extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final pref = ref.watch(appFontPrefProvider);
    final controller = ref.read(appFontPrefProvider.notifier);
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 2),
                    Text(
                      'جرّب الخطوط واختر الأنسب — يتبدّل التطبيق فورًا.',
                      style: TextStyle(
                        color: AppTheme.faintMuted,
                        fontSize: 11.5,
                        fontWeight: FontWeight.w600,
          ...
          const SizedBox(height: 14),
          for (final option in AppFontPref.values) ...[
            _FontOption(
              option: option,
              active: pref == option,
              onTap: () => controller.setPref(option),
            ),
            const SizedBox(height: 8),
          ],
          const SizedBox(height: 2),
          // Honest note so the user knows google_fonts downloads on
          // first preview.
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'بعض الخطوط تُجلَب من Google Fonts عند أول معاينة، '
                    'ثم تُحفظ على الجهاز للاستخدام بدون إنترنت.',
                    style: TextStyle(
                      color: AppTheme.indigoPrimary,
                      fontSize: 11,
...
class _FontOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Each row carries:
    //   * radio dot on the leading edge
    //   * font name (rendered in the system font for clarity)
    //   * sample sentence (rendered in the FONT this row represents
    //     — that's the point of the previewer).
    final labelStyle = option.toTextStyle(
    ...
    final sampleStyle = option.toTextStyle(
      base: const TextStyle(
          ...
          decoration: BoxDecoration(
            gradient: active
                ? LinearGradient(
                    ...
                    colors: [
                      AppTheme.indigoPrimary.withValues(alpha: 0.10),
            ...
            border: Border.all(
              color: active
                  ? AppTheme.indigoPrimary.withValues(alpha: 0.45)
                  : AppTheme.line,
              width: active ? 1.4 : 1,
            ),
            boxShadow: active
                ? [
                    BoxShadow(
                      color: AppTheme.indigoPrimary.withValues(alpha: 0.14),
                      blurRadius: 12,
                      offset: const Offset(0, 4),
                    ),
                  ]
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(option.arabicLabel, style: labelStyle),
                      const SizedBox(height: 4),
                      Text(option.sampleText, style: sampleStyle),
                    ],
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: active ? AppTheme.indigoPrimary : Colors.white,
        border: Border.all(
          color: active ? AppTheme.indigoPrimary : AppTheme.line,
          width: active ? 0 : 1.4,
        ),
        boxShadow: active
            ? [
